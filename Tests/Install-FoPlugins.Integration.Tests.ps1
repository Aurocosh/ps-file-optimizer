...
Describe 'Install-FoPlugins integration' -Tag Integration -Skip:(-not $env:FO_RUN_INSTALL_INTEGRATION) {
    ...
    It 'Downloads DSSIM when -Component Dssim on 64-bit PowerShell' -Skip:(-not [Environment]::Is64BitProcess) {
        ...
        try {
            $result = Install-FoPlugins -Component Dssim -DestinationPath $dest -TempDirectory $tempRoot -Force
